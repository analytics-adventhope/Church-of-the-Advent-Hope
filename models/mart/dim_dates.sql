...
select distinct date(d.date) as date,
       ...
       case when date_diff(m.max_end_date, d.date, day) / 30 <= 3  then 'a. Months 0 - 3'
            when date_diff(m.max_end_date, d.date, day) / 30 <= 6  then 'b. Months 4 - 6'
            when date_diff(m.max_end_date, d.date, day) / 30 <= 12 then 'c. Months 7 - 12'
            when date_diff(m.max_end_date, d.date, day) / 30 <= 24 then 'd. Months 13 - 24'
            else 'e. Months 25+'
       end as time_bucket
from days d
